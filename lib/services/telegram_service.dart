  ...
  Future<void> _pollUpdates() async {
    ...
    try {
      ...
      if (response.statusCode == 200) {
        _consecutiveFailures = 0; // Reset watchdog
    ...
    // Watchdog Healing Service: Exponential Backoff
    if (_isPolling) {
      int delaySeconds = 1;
      if (_consecutiveFailures > 0) {
        // Exponential backoff capped at 60 seconds
        delaySeconds = (1 << _consecutiveFailures).clamp(1, 60);
        print('Watchdog: Polling failed, retrying in $delaySeconds seconds...');
      }
